...
#include "lgvRouting/common/argh.h"
...
std::string CmdParser::addArg(std::string name, std::string default_val, std::string info) {
    if(name.size() <= 1 || name[0] == '-') {
        lgvERR(std::string{"arguments should not start with '-', this is not valid: "}+name+"\n");
        exit(1);
    }
    ...
    for(int i=0; i<args.size(); i++) {
        if(args[i].default_val.size() > 1) {
            lgvERR("You cant add an Arg after an ArgList\n");
            exit(1);      
        }
    }
...
bool CmdParser::addBoolOpt(std::string opt, std::string info) {
    if(opt.size() <= 1 || opt[0] != '-') {
        lgvERR(std::string{"option must start with '-', this is not valid: "}+opt+"\n");
        exit(1);
    }
    ...
    if(!cmdl(opt).str().empty()) {
        lgvERR(std::string{"you cant append values to boolean flag: "}+opt+"\n");
        exit(-1);
    }
...
void CmdParser::parse() {
    ...
    for (std::string flag : flags) {
        ...
        if(!found) {
            lgvERR(std::string{"flag not valid: -"}+flag+"\n");
